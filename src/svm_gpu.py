...
class SVM():
    ...
    def fit(self, x=None, y=None, prevent_relabel=False, use_optimal_lambda=False):
        ...
        if self._use_optimal_lambda or use_optimal_lambda:
            ...
            print('Misclassification error (train), {}, optimal lambda = {} : {}'.format(self._classification_strategy,
                                                                                         self._lambduh,
    ...
    def compute_optimal_lambda(self):
        ...
        if self._display_plots:
            df = pd.DataFrame(
                {'lambda': self._lambda_vals, 'Cross validation error': xp.asnumpy(cross_validation_error)})
            display(df)
    ...
    def plot_misclassification_error(self):
        if self._classification_strategy == 'binary':
            ...
            ax.plot(np.array(range(len(self._misclassification_error_per_iter))) * 10,
                    xp.asnumpy(self._misclassification_error_per_iter), label='Train', c='red')
            plt.xlabel('Iteration')
    ...
    def _compute_gradient(self, alpha):
        ...
        grad = -2 / self._n * xp.sum(
            self._y[:, xp.newaxis] * self._K * xp.max(xp.stack((xp.zeros_like(self._y), 1 - self._y * K_alpha)),
                                                      axis=0)[:, xp.newaxis], axis=0) + 2 * self._lambduh * K_alpha
        return grad

    def _objective(self, alpha):
        K_alpha = xp.dot(self._K, alpha)
        return 1 / self._n * xp.sum(
            xp.max(xp.stack((xp.zeros_like(self._y), 1 - self._y * K_alpha)), axis=0) ** 2) + self._lambduh * alpha.dot(
            K_alpha)
    ...
    def _backtracking_line_search(self, alpha, eta=1, alphaparam=0.5, betaparam=0.8, max_iter=100):
        ...
        while found_eta == 0 and iter < max_iter:
            if self._objective(alpha - eta * grad_alpha) < self._objective(
                    alpha) - alphaparam * eta * norm_grad_alpha ** 2:
                found_eta = 1
            elif iter == max_iter:
    ...
    def _fast_gradient_descent(self):
        ...
        while iter < self._max_iter:
            ...
            if self._display_plots and iter % 10 == 0:
                ...
                misclassification_error_per_iter[int(iter / 10)] = self.compute_misclassification_error(self._x,
                                                                                                        xp.where(
                                                                                                            self._y > 0,
                                                                                                            self._primary_class,
    ...
    def _optimal_eta_init(self):
        return 1 / scipy.linalg.eigh(xp.asnumpy(2 / self._n * xp.dot(self._K, self._K) + 2 * self._lambduh * self._K),
                                     eigvals=(self._n - 1, self._n - 1), eigvals_only=True)[0]
